---
title: "**Visualisation of Covid Data**"
...
Covid19DeathsTotal <-  vroom("../data/covidData/time_series_covid19_deaths_global.csv")


## Changing working directory, in case local files needed
...
## I need to assign the long version of my data to a new variable, so I still have access to the original files
...
NumberOfCountries <- length((unique(Covid19DeathsTotalLong$Country_Region)))
...
This graph is plotted as each countries increase in deaths byCovidover the time frame within the data frame. The graph is messy, but I can see between April and March that the initial peak began.
```{r, echo=FALSE}

## make the ggplot object
FirstGraphicalAnalysis <- ggplot(data = Covid19DeathsTotalLong, aes(x = Date_Corrected, y = Deaths, "FirstGraphicalAnalysis")) + geom_point(aes(col = Country_Region)) + theme(legend.position = "none")+ ggtitle ("Graphical Analysis of CovidTime Stamp Data, for the whole duration")+  labs(y = "Total Deaths", x = "Date")
...
This graph shows the period of 2020-03-20 to 2020-06-01, which is the initial spike inCoviddeaths.
...
SecondGraphicalAnalysis + geom_point(aes(col = Country_Region)) + theme(legend.position = "none")+ ggtitle ("Graphical Analysis of CovidTime Stamp Data, for the Initial Peak" ) +                                       
...
```{r, include = FALSE}
## For my interest into the population data, I pulled the populations from 2019-2020 from the package wbstats, converted this to a tibble. I select the date range that covered the entirety ofCovidup to the present day
...
```{r, echo=FALSE,message=FALSE,warning=FALSE}
## Up to dateCoviddeath data for the UK, from https://coronavirus.data.gov.uk/details/deaths
CovidCountryDataUKForDeaths = vroom("../data/covidData/overview_2021-10-18.csv")
...
UKHospitalAndDeathDailyData <-  vroom("../data/covidData/HospitalAndDailyDeathCovidData.csv")
...
This graph is taken from constantly updating Covid19 death data for the last 28 days, hover over a point to find out information.
...
## This is responsible for embedding the graph into a shiny output, so the hovering points actually work
...
WorldInformation <-   vroom("../data/covidData/WorldCovidData.csv")
...
) +ggtitle("World Heat Map ofCovidDeath Totals")
